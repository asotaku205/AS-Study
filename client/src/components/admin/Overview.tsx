...
const MONOCHROME_COLORS = ['#0f172a', '#334155', '#64748b', '#94a3b8', '#cbd5e1', '#1e293b', '#475569'];
...
const Overview = ({user,docs}: { user: User[]; docs: Document[] }) => {
  ...
  useEffect(() => {
    ...
    const loadStats = async () => {
      try {
        const [quizData, growthData, catData] = await Promise.all([
          getAdminQuizStats(),
  ...
  // Compute recent activity from real docs data
  ...
  // Compute document status distribution for bar chart
  const statusCounts = docs.reduce<Record<string, number>>((acc, d) => {
    acc[d.status] = (acc[d.status] || 0) + 1;
    return acc;
  }, {});
  const docStatusData = Object.entries(statusCounts).map(([name, val]) => ({ name, val }));
     ...
     <div className="space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-500">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        <div className="lg:col-span-2 bg-white/80 dark:bg-slate-900/80 backdrop-blur-2xl p-8 rounded-2xl border border-white/50 dark:border-slate-800 shadow-xl flex flex-col">
          <div className="flex items-center justify-between mb-8">
            ...
            <span className="text-sm font-semibold text-slate-500 dark:text-slate-400">7 ngày qua</span>
          </div>
          <div className="flex-1 min-h-[260px] w-full">
            {userGrowthData.length > 0 ? (
              <ResponsiveContainer width="100%" height="100%">
                <AreaChart data={userGrowthData} margin={{ top: 15, right: 15, left: -20, bottom: 15 }}>
                  <defs>
                  ...
                  <YAxis axisLine={false} tickLine={false} tick={{ fill: isDark ? "#64748B" : "#94A3B8", fontSize: 13, fontWeight: 600 }} allowDecimals={false} />
              ...
              <div className="flex items-center justify-center h-full text-slate-400 dark:text-slate-500 text-sm font-medium">Đang tải dữ liệu...</div>
            )}
          </div>
        </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        <div className="bg-white/80 dark:bg-slate-900/80 backdrop-blur-2xl p-6 rounded-2xl border border-white/50 dark:border-slate-800 shadow-xl">
          <h3 className="font-bold text-slate-900 dark:text-white text-lg mb-5">Phân bổ trạng thái tài liệu</h3>
          <div className="min-h-[200px]">
            {docStatusData.length > 0 ? (
              <ResponsiveContainer width="100%" height={200}>
                <BarChart data={docStatusData} margin={{ top: 5, right: 10, left: -20, bottom: 0 }}>
                  <CartesianGrid strokeDasharray="3 3" vertical={false} stroke={isDark ? "#1E293B" : "#E2E8F0"} />
                  <XAxis dataKey="name" axisLine={false} tickLine={false} tick={{ fill: isDark ? "#64748B" : "#94A3B8", fontSize: 12, fontWeight: 600 }} dy={10} />
                  <YAxis axisLine={false} tickLine={false} tick={{ fill: isDark ? "#64748B" : "#94A3B8", fontSize: 12, fontWeight: 600 }} allowDecimals={false} />
                  ...
                  <Bar dataKey="val" name="Số tài liệu" fill={isDark ? "#F8FAFC" : "#0F172A"} radius={[4, 4, 0, 0]} />
                </BarChart>
              </ResponsiveContainer>
            ) : (
              <div className="flex items-center justify-center h-[200px] text-slate-400 dark:text-slate-500 text-sm font-medium">Chưa có dữ liệu</div>
            )}
          </div>
        </div>
              ...
              <div key={idx} className="flex items-center gap-3 p-3 rounded-xl hover:bg-slate-50 dark:hover:bg-slate-800/50 transition-colors">
                ...
                <div className="flex-1 min-w-0">
                  <p className="text-sm font-medium text-slate-800 dark:text-slate-200 truncate">
                    {doc.status === "Pending" ? "Tài liệu chờ duyệt" : doc.status === "Published" ? "Tài liệu đã xuất bản" : `Tài liệu ${doc.status}`}: {doc.title}
                  </p>
                  <p className="text-xs text-slate-400 dark:text-slate-500 font-medium">{formatTimeAgo(doc.createdAt)}</p>
                </div>
              </div>
            )) : (
              <p className="text-sm text-slate-400 dark:text-slate-500 italic">Chưa có hoạt động nào.</p>
            )}
          </div>
        </div>
      </div>
